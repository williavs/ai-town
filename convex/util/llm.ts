...
// Set to match your embedding model's output dimension.
// OpenAI text-embedding-3-small = 1536, Together m2-bert = 768, Ollama mxbai = 1024
...
// Like chatCompletion but returns the full choice object including tool_calls
export async function chatCompletionRaw(
  body: Omit<CreateChatCompletionRequest, 'model'> & {
    model?: CreateChatCompletionRequest['model'];
  },
): Promise<{ choice: ChatCompletionChoice; retries: number; ms: number }> {
  const config = getLLMConfig();
  body.model = body.model ?? config.chatModel;
  const stopWords = body.stop ? (typeof body.stop === 'string' ? [body.stop] : body.stop) : [];
  if (config.stopWords) stopWords.push(...config.stopWords);
